...
import com.vigoredu.databinding.ActivityMainBinding;
import com.vigoredu.network.ApiHelper;
import com.vigoredu.network.NetworkError;
...
public class MainActivity extends BaseActivity {
    ...
    private void asyncAPI() {
        if (Utility.isNetworkAvailable(true)) {
            showProgressDialog();
            mCompositeDisposable.add(ApiHelper.performUpdateCoordinates().
                    subscribeOn(Schedulers.newThread()).
                    observeOn(AndroidSchedulers.mainThread()).
                    subscribe(response -> {
                        hideProgressDialogue();
                        if (response != null && response.isStatus() && response.getCode() == 200) {
//                            setData(response.getData());
                        ...
                        hideProgressDialogue();
                        Utility.showMessageL(NetworkError.getErrorMessage(throwable));
                    }));
        } else
